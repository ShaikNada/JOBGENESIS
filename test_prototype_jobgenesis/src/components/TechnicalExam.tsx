...
export const TechnicalExam = ({ role, company, level, focus, difficulty = 'normal', candidateId, onFinish }: TechnicalExamProps) => {
    ...
    const handleProctorStrike = (reason: string) => {
        ...
        toast.error(`⚠️ INTEGRITY BREACH: ${reason} (-5 PTS)`);
        
        if (candidateId) {
            getSocket().emit('candidate_telemetry', { id: candidateId, risk: 'Medium' });
            getSocket().emit('candidate_log', { 
                log: `> [OVERWATCH] Proctor Strike on Candidate ${candidateId} during MCQ: ${reason}` 
            });
        }
    };
    ...
    useEffect(() => {
        if (disqualified) {
            toast.error("PROTOCOL TERMINATED: Integrity Breach Limit Reached", { duration: 5000 });
            setTimeout(() => {
                onFinish(0); // Disqualified score
            }, 2000);
        }
    }, [disqualified]);
        ...
        return (
            <div className="h-screen bg-dark-950 flex flex-col items-center justify-center text-white font-mono p-6 text-center overflow-y-auto">
                <div className="max-w-md w-full bg-dark-900 border border-dark-700 p-8 rounded-2xl shadow-[0_0_50px_rgba(0,0,0,0.5)] animate-in zoom-in-95 my-auto">
                    ...
                    <div className="mb-8">
                        <div className="text-6xl font-black text-white mb-2">{examScore}/{questions.length}</div>
                        <div className="text-[10px] uppercase tracking-widest text-dark-500 font-black mb-4 flex items-center justify-center gap-2">
                            <ShieldAlert size={12} className={integrityScore < 100 ? 'text-neon-red' : 'text-green-500'} />
        ...
        <div className="h-screen bg-dark-950 flex flex-col items-center text-white font-mono p-6 overflow-y-auto custom-scrollbar">
            <div className="w-full max-w-3xl animate-in fade-in slide-in-from-bottom-4 py-8">
                ...
                <div className="mt-6 flex justify-between text-[10px] text-dark-600 font-bold uppercase tracking-[0.3em]">
                    ...
                    <span className={integrityScore < 100 ? 'text-neon-red font-black' : ''}>
                        Overwatch Status: {integrityScore < 100 ? '⚠️ BREACH_DETECTED' : 'ACTIVE'}
                    </span>
