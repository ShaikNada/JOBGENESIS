...
export const JobDashboard = ({ user, resumeData, onStartSimulation, onViewProfile, onUploadResume, onLogout, isInvestor = false }: JobDashboardProps) => {
    ...
    const handleScanTarget = async () => {
        ...
        try {
            ...
            const [pathRes, gapRes] = await Promise.allSettled([
                ...
                fetch(`${API_URL}/api/jobs/target-path`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    ...
                    <div className="flex justify-between items-center bg-[#0a0a0f] p-4 rounded-2xl border border-white/5 tactical-border">
                        <div className="flex items-center gap-6">
                            <div className="relative">
                                ...
                                <div className="absolute -bottom-2 -right-2 px-2 py-0.5 bg-neon-red text-[8px] font-black italic rounded border border-[#020202]">
                                    LEVEL {Math.floor((user?.skillTree?.totalXP || 0) / 100) + 1}
                                </div>
                            </div>
                            <div>
                                <h1 className="text-2xl font-black tracking-tighter text-white uppercase italic leading-none mb-1">
                                    OPERATIVE: {user?.name || 'CANDIDATE'}
                                </h1>
                                <div className="flex items-center gap-3">
                                    ...
                                    <span className="text-[10px] font-black text-gray-500 tracking-widest uppercase">
                                        Rank: { (user?.skillTree?.totalXP || 0) > 5000 ? 'Platinum Prime' : (user?.skillTree?.totalXP || 0) > 1000 ? 'Gold Vanguard' : 'Initiate' }
                                    </span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="col-span-12 lg:col-span-4 space-y-6 text-left">
                                ...
                                <div className="space-y-6">
                                    <div>
                                        <div className="text-3xl font-black text-white italic tracking-tighter mb-1">
                                            {isInvestor ? "94%" : (user?.matchRate || "0%")}
                                        </div>
                                        <div className="text-[9px] text-gray-600 uppercase tracking-widest font-black">System Match Rate</div>
                                    </div>
                                    <div>
                                        <div className="text-3xl font-black text-[#ff0044] italic tracking-tighter mb-1">
                                            {(user?.skillTree?.totalXP || 0).toLocaleString()} <span className="text-xs">XP</span>
                                        </div>
                                    ...
                                    <div className="pt-6 border-t border-white/5 space-y-4">
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {(resumeData?.skills || ['Identity Pending', 'Scan Required']).slice(0, 5).map((skill: string) => (
                            ...
                            <div className="bg-[#0a0a0f] border border-white/5 rounded-[32px] p-8 relative overflow-hidden group phantom-card">
                                ...
                                <div className="space-y-4">
                                    {[
                                        { label: 'Frontend', val: user?.skillTree?.frontend || 0, color: '#00f0ff' },
                                        { label: 'Backend', val: user?.skillTree?.backend || 0, color: '#a855f7' },
                                        { label: 'Security', val: user?.skillTree?.security || 0, color: '#ff0044' }
                                    ].map(item => (
                                        <div key={item.label}>
                                            <div className="flex justify-between text-[10px] font-black uppercase mb-2">
                                                <span className="text-gray-500">{item.label}</span>
                                                <span style={{ color: item.color }}>{item.val >= 100 ? 'MAX' : `${item.val}%`}</span>
                                            </div>
